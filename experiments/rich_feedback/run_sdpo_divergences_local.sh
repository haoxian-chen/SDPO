...
# Usage: ./run_sdpo_divergences_local.sh [--dry-run]
#
# Slurm-less variant of experiments/rich_feedback/run_sdpo_divergences.sh.
# Same DIVERGENCE_TYPES sweep on LiveCodeBench v6, but each "job" is a
# foreground invocation of training/verl_training.sh on the current node,
# run one after another. Use when:
