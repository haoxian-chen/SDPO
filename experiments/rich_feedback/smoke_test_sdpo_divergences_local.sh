...
# Usage: ./smoke_test_sdpo_divergences_local.sh [--dry-run]
#
# Local (no-Slurm) smoke test for the LCB divergence sweep. Runs 10 outer
# steps for one divergence (jsd by default — exercises the new clamp +
# log1p code path in _compute_neg_g_u) to confirm the new sampled-token
# code path works end-to-end before launching the full
# run_sdpo_divergences_local.sh sweep.
#
...
#
# Differences vs run_sdpo_divergences_local.sh:
...
#   - rollout.val_kwargs.n=2 (cheaper validation)
#   - EXP_NAME prefix SMOKE-, group_name suffix -smoke
#
# Expected wall-clock: ~7-20 min on 4 GPUs (Qwen3-8B).
...
# Sweep Parameters (mirror run_sdpo_divergences.sh)
...
# Allow caller to override GPU count for n_gpus_per_node without editing yaml.
...
run_job() {
    ...
    echo "  log: $log_file"
    # Tee output to a log so the user sees progress and has a persistent record.
...
trainer.group_name=SDPO-rich-feedback-div-smoke \
