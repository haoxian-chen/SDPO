...
# Usage: ./run_sdpo_tooluse_divergences.sh [--dry-run]
#
# Sweep SDPO on ToolUse across the 5 f-divergences added to
# SelfDistillationConfig.divergence_type (sampled-token branch). All other
# hyperparameters mirror experiments/generalization/run_sdpo_all.sh; the
# only loss-relevant changes are full_logit_distillation=False and
# divergence_type. SciKnowEval data paths are dropped here per scope.
# See plans/sdpo_run_divergences_experiments.md.
                ...
                for MODEL_PATH in "${MODEL_PATHS[@]}"; do
                    for DIVERGENCE_TYPE in "${DIVERGENCE_TYPES[@]}"; do
                        for DATA_PATH in "${DATA_PATHS[@]}"; do
                            ...
                            EXP_NAME="FINAL-SDPO-train${TRAIN_BATCH_SIZE}-div${DIVERGENCE_TYPE}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-dross${DONTS_REPROMPT_ON_SELF_SUCCESS}-${MODEL_NAME}"
